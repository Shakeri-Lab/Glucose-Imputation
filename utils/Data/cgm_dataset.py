...
class CGMDataset(Dataset):
    def __init__(self, data_path, seq_len=288, stride=12, 
        ...
        if is_pedap:
            ...
            self.df.rename(columns={'bolus_extended': 'bolus'}, inplace=True)
            self.df['date'] = pd.to_datetime(self.df['date'], format='%Y-%m-%d %H:%M:%S')
            self.nane_cgm = pd.read_csv(self.miss_cfg['nane_cgm_dir'])
    ...
    def _build_samples_pedap(self):
        ...
        for pid, group in self.df.groupby('pat_id'):
            for seq_id, episode in group.groupby('seq_id'):
                if self.miss_cfg['type'] == 'A':
                    nane_episode = self.nane_cgm[
                    (self.nane_cgm['pat_id'] == pid) & 
                    (self.nane_cgm['seq_id'] == seq_id)
                    ]
                else: 
                    nane_episode = None
                samples.extend(self._generate_samples(episode, nane_episode))
        return np.array(samples)
        
    def _generate_samples(self, group, nane_episode=None):
        samples = []
        num_points = len(group)
        for i in range(0, num_points - self.seq_len + 1, self.stride):
            ...
            if self.missing_enabled:
                if self.miss_cfg['is_real_mask']: 
                    ...
                    slice_df = simulate_experiment_pipeline(group.iloc[i : i + self.seq_len], nane_episode.iloc[i : i + self.seq_len] if nane_episode is not None else None, self.miss_cfg)
